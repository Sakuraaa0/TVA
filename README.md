...
- gMark (`sf1`, bibliographical schema): vertices 100,000, edges 239,737 (loaded: 92,953 edges, 56,889 nodes)

  - Base graph generated by the [gMark](https://github.com/gbagan/gmark) tool with `size=sf1` (bibliographical schema). The graph file `wd-graph.txt` uses format `src predicate dst` with predicate IDs 0–81. Edges are split on load: predicates 51–81 go to the relation store (75,417 edges), predicates 0–50 go to the attribute column store (17,536 edges, 51 attribute types).

  - Graph update operations (`graph_op/cypher.txt`) generated by `create_graph_op_queries.py`:

    ```
    python create_graph_op_queries.py --size sf1 --num-op 80000 --update-ratio 1.0 --delete-ratio 0.0 --write-path ../../results/
    ```

    Node IDs for updates are sampled via Zipf distribution (a=1.1). Output is Cypher `MATCH (c:Node {id: 'X'}) SET c.name = Y;` statements.

  - Temporal queries (`temporal_query/cypher.txt`) generated by `create_temporal_query.py`:

    ```
    python create_temporal_query.py --query-size 5 --min-time 0 --max-time 100 --write-path ../../results/
    ```

    Takes 5 original gMark query files and appends `TT AS <random_timestamp>` to produce 50 temporal query statements.
